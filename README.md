...
- 3 friendly mesh networks (Alpha / Bravo / Charlie), each with a command node, infantry, armor, and recon units that wander realistically around their sectors. 4 OPFOR tracks.
...
SQLite (`backend/takdemo.db`) tables: `units`, `cot_events`, `message_log`, `link_snapshots`.
